...
缺点: 占用较多的空间，因为其在类加载时就会完成实例化,不能延时加载.
...
* 实现了延时加载并且保证了执行的效率,利用JVM的机制完成的.
* 当我们不行获取单例类的实例而想获取单例类的一个全局变量时,可以使用静态内部类.
* 只有我们显式的调用getInstance方法,才会加载SingletonHolder类，从而实例化instance.
* JVM保证类的静态成员只能被加载一次的特点
...
__建议使用饿汉方式,只有在要明确实现 lazy loading(延时加载) 效果时，才会使用静态内部类模式。如果涉及到反序列化创建对象时，可以尝试使用种枚举方式或者加入readResolve方法__
